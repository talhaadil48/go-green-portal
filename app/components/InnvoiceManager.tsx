...
export default function InvoiceManager({ claimId }: InvoiceManagerProps) {
    ...
    // Fetch all form data to check availability
    useEffect(() => {
    ...
    const documents: DocumentOption[] = [
        ...
        },
        {
            ...
            icon: (
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                </svg>
            ),
        ...
        },
        {
            ...
            icon: (
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7v8a2 2 0 002 2H5a2 2 0 00-2 2v6a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" />
                </svg>
            ),
    ...
    const handleSendDocuments = async () => {
        ...
        setIsSending(true);
        setStatus({ type: "info", text: "Generating PDFs and sending email..." });

        try {
            // Generate all selected PDFs or fetch documents
            const pdfBlobs: { blob: Blob; filename: string; formType: string }[] = [];
            ...
            for (const docId of selectedDocs) {
                ...
                if (doc.formType === "document") {
                    ...
                    if (!documentsData[docId]) continue;
            ...
            // Send all PDFs via API
            const formData = new FormData();
            pdfBlobs.forEach((pdf, index) => {
                formData.append(`file_${index}`, pdf.blob, pdf.filename);
            });
            formData.append('fileCount', String(pdfBlobs.length));
            formData.append('email', email);
            formData.append('subject', subject);
            formData.append('message', message);
            formData.append('claimId', claimId);
            formData.append('documentTypes', selectedDocs.join(','));
            ...
            if (data.success) {
                setStatus({
                    type: "success",
                    text: data.message || `Successfully sent ${pdfBlobs.length} separate email(s) to ${email}!`
                });
            } else {
                setStatus({
                    type: "error",
                    text: data.message || "Some or all emails failed to send."
                });
            }
            setSelectedDocs([]);
            setEmail("");
            ...
            setStatus({ type: "error", text: "Failed to send documents. Please try again." });
        } finally {
            setIsSending(false);
        }
    };
    ...
    const extractSignatures = (docId: string, data: any): Record<string, string | null> => {
        ...
        switch (docId) {
            ...
            case 'rental-agreement':
                ...
                break;
            default:
                // For documents or unknown, return empty
                break;
        }
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="text-center">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Send Invoice & Documents</h2>
                <p className="text-gray-600">Select the documents you want to send and enter the recipient's email</p>
            </div>
                    ...
                    {allDocuments.map((doc) => {
                        ...
                        return (
                            <button
                                ...
                                className={`relative group text-left p-5 rounded-2xl border-2 transition-all duration-300 ${!isAvailable
                            ...
                            >
                                {/* Checkbox */}
                                <div
                                    className={`absolute top-4 right-4 w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${isSelected
                                ...
                                {/* Icon */}
                                <div
                                    className={`w-12 h-12 rounded-xl flex items-center justify-center mb-3 transition-all ${isSelected
                                ...
                                {/* Content */}
                                <h4 className={`font-bold mb-1 ${isSelected ? "text-emerald-800" : "text-gray-800"}`}>
                                    {doc.name}
                                </h4>
            ...
            {status && (
                <div
                    className={`p-4 rounded-2xl flex items-center gap-3 ${status.type === "success"
            ...
            <div className="flex justify-center">
                ...
                >
                    ...
                    {isSending ? (
                        <>
                            <svg className="w-6 h-6 animate-spin relative z-10" fill="none" viewBox="0 0 24 24">
                                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                            </svg>
            ...
            <div className="text-center text-sm text-gray-500">
                <p>All documents will be professionally formatted as PDF attachments</p>
                <p className="mt-1">with your company branding and proper formatting</p>
            </div>
        </div>
    );
}
